...
function buildPrefsFromForm() {
  ...
  console.log("Preferences being sent:", prefs);
  return prefs;
}
...
function renderMatches(results) {
  ...
  if (!results || results.length === 0) {
    resultsList.innerHTML = `
      <div class="muted">
        No matches showing yet. Choose preferences and click Find Matches.
      </div>
    `;
    return;
  }
      ...
      <div class="resultCard__top">
        <h3 class="resultCard__name">
          <a 
            ...
            href="storepage.html?locationId=${Number(place.id)}&name=${encodeURIComponent(place.name)}"
          >
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    if (resultsList) {
      resultsList.innerHTML = `<div class="muted">Loading matches...</div>`;
    }
...
if (btnClearMatches) {
  btnClearMatches.addEventListener("click", () => {
    if (!confirm("Clear all matches and start fresh?")) return;
    ...
    if (resultsList) {
      resultsList.innerHTML = `
        <div class="muted">
          Matches cleared. Choose your preferences and click Find Matches again.
        </div>
      `;
    }
  });
